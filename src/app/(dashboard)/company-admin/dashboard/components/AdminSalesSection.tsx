...
export function AdminSalesSection() {
  ...
  const [form, setForm] = useState({ name: '', country: '', system: '', connectedBy: '', customerEmail: '', alternateContactNo: '', customerAddress: '', plan: '', paymentMerchant: '', amount: 0, paymentMethod: 'Other' as ICompanySale['paymentMethod'], saleDate: getBusinessDateString() });
  ...
  const openAdd = () => { setEditing(null); setForm({ name: '', country: '', system: '', connectedBy: '', customerEmail: '', alternateContactNo: '', customerAddress: '', plan: '', paymentMerchant: '', amount: 0, paymentMethod: 'Other', saleDate: getBusinessDateString() }); setAdding(true); };
  const openEdit = (sale: ICompanySale) => { setEditing(sale); setForm({ name: sale.name, country: sale.country, system: sale.system, connectedBy: sale.connectedBy, customerEmail: sale.customerEmail || '', alternateContactNo: sale.alternateContactNo || '', customerAddress: sale.customerAddress || '', plan: sale.plan || '', paymentMerchant: sale.paymentMerchant || '', amount: sale.amount, paymentMethod: sale.paymentMethod, saleDate: sale.saleDate }); setAdding(true); };
  ...
  return <section className="min-h-full space-y-5 overflow-y-auto bg-slate-950 p-6 text-slate-100"><header className="flex items-center justify-between border-b border-slate-800 pb-4"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-white"><TrendingUp className="h-6 w-6 text-emerald-400" /> Sales Revenue</h1><p className="text-sm text-slate-400">Manage company sales transactions.</p></div><button onClick={openAdd} className="flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white"><Plus className="h-4 w-4" />Add Sale</button></header>
    ...
    {adding && <form onSubmit={save} className="grid gap-3 rounded-xl border border-indigo-500/30 bg-slate-900 p-4 md:grid-cols-4">
      ...
      <div className="flex items-end gap-2 md:col-span-4"><button className="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white">Save</button><button type="button" onClick={() => setAdding(false)} className="rounded-lg bg-slate-700 px-3 py-2 text-xs text-white">Cancel</button></div>
    </form>}
    <div className="overflow-x-auto rounded-xl border border-slate-800"><table className="w-full text-left text-xs text-slate-300"><thead className="bg-slate-900 text-slate-400"><tr>{['Customer & ID', 'Contact & Address', 'System & Plan', 'Closed By', 'Merchant & Payment', 'Date', 'Amount', 'Actions'].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-800">{filtered.map((sale) => <tr key={sale._id} className="hover:bg-slate-900/40 transition-colors"><td className="p-3"><div className="flex items-center gap-1.5"><b className="font-semibold text-white text-sm">{sale.name}</b>{sale.customerId && <span className="rounded bg-indigo-500/20 px-1.5 py-0.5 text-[10px] font-mono font-bold text-indigo-300 border border-indigo-500/30">#{sale.customerId}</span>}</div><span className="block text-slate-500 text-[11px]">{sale.country}</span></td><td className="p-3 text-[11px]"><div className="text-slate-300">{sale.customerEmail || '—'}</div>{sale.alternateContactNo && <div className="text-emerald-400">Alt: {sale.alternateContactNo}</div>}{sale.customerAddress && <div className="text-slate-500 truncate max-w-xs">{sale.customerAddress}</div>}</td><td className="p-3"><div className="font-medium text-slate-200"><Building2 className="mr-1 inline h-3.5 w-3.5 text-slate-400" />{sale.system}</div>{sale.plan && <div className="text-[11px] text-cyan-400">{sale.plan}</div>}</td><td className="p-3">{sale.connectedBy}</td><td className="p-3"><div><span className="font-medium text-slate-200">{sale.paymentMethod}</span></div>{sale.paymentMerchant && <span className="text-[10px] text-slate-400 block font-mono">Via: {sale.paymentMerchant}</span>}</td><td className="p-3 text-slate-400">{sale.saleDate}</td><td className="p-3 text-emerald-400 font-mono font-bold text-sm">${sale.amount.toLocaleString()}</td><td className="p-3"><button onClick={() => openEdit(sale)} className="mr-2 text-indigo-400" title="Edit sale"><Pencil className="inline h-4 w-4" /></button><button onClick={() => openMarkFailed(sale)} className="mr-2 text-amber-400" title="Mark sale as failed"><Flag className="inline h-4 w-4" /></button><button onClick={() => void remove(sale._id)} className="text-rose-400" title="Delete sale"><Trash2 className="inline h-4 w-4" /></button></td></tr>)}</tbody></table></div>
  </section>;
}
