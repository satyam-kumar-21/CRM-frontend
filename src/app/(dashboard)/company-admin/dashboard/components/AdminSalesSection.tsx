...
export function AdminSalesSection() {
  ...
  const [form, setForm] = useState({ name: '', country: '', system: '', connectedBy: '', amount: 0, paymentMethod: 'Other' as ICompanySale['paymentMethod'], saleDate: getBusinessDateString() });
  ...
  const openAdd = () => { setEditing(null); setForm({ name: '', country: '', system: '', connectedBy: '', amount: 0, paymentMethod: 'Other', saleDate: getBusinessDateString() }); setAdding(true); };
  ...
  const openMarkFailed = (sale: ICompanySale) => { setMarkingFailedSale(sale); setFailedReason(''); };
  const cancelMarkFailed = () => { setMarkingFailedSale(null); setFailedReason(''); };
  const confirmMarkFailed = async () => {
    if (!markingFailedSale) return;
    if (!failedReason.trim()) { toast.error('Enter a failed reason'); return; }
    try {
      await companyService.markSaleFailed(markingFailedSale._id, failedReason.trim());
      setSales((current) => current.filter((sale) => sale._id !== markingFailedSale._id));
      setMarkingFailedSale(null);
      setFailedReason('');
      toast.success('Sale marked as failed');
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Unable to mark sale as failed');
    }
  };
  ...
  return <section className="min-h-full space-y-5 overflow-y-auto bg-slate-950 p-6 text-slate-100"><header className="flex items-center justify-between border-b border-slate-800 pb-4"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-white"><TrendingUp className="h-6 w-6 text-emerald-400" /> Sales Revenue</h1><p className="text-sm text-slate-400">Manage company sales transactions.</p></div><button onClick={openAdd} className="flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white"><Plus className="h-4 w-4" />Add Sale</button></header>
    ...
    {markingFailedSale && <div className="rounded-xl border border-rose-500/40 bg-rose-950/20 p-4">
      <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-semibold text-white">Mark Sale as Failed</h2><p className="text-sm text-slate-400">Sale will move to Failed Sales and will no longer appear in normal sales.</p></div><button type="button" onClick={cancelMarkFailed} className="rounded-lg bg-slate-700 px-3 py-2 text-xs text-white"><X className="inline h-4 w-4" /></button></div>
      ...
      <label className="mt-4 block text-xs text-slate-400"><span>Reason</span><input type="text" value={failedReason} onChange={(event) => setFailedReason(event.target.value)} placeholder="Customer returned product, Chargeback, Payment failed..." className={`${input} mt-1 w-full`} /></label>
      <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={confirmMarkFailed} className="rounded-lg bg-rose-600 px-3 py-2 text-xs font-semibold text-white">Confirm failed sale</button><button type="button" onClick={cancelMarkFailed} className="rounded-lg bg-slate-700 px-3 py-2 text-xs text-white">Cancel</button></div>
    </div>}
