...
type WorkspaceNotificationWatcherProps = { dashboardPath: string };
...
export function WorkspaceNotificationWatcher({ dashboardPath }: WorkspaceNotificationWatcherProps) {
  useEffect(() => {
    ...
    const onMessage = (message: ICompanyMessage) => {
      if (!message?._id || message.isMine || !('Notification' in window) || Notification.permission !== 'granted') return;
